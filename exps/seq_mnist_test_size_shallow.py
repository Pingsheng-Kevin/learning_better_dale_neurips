...
results_dir = PLAYGROUND_DIR/f"seq_mnist_test_{model_type}_shallow_icml"
...
hidden_units = [1000, 300, 100]
seeds = [0,1,2,3,4]
# rad_list = [0.85, 0.98, 0.89]
# para_comb = list(itertools.product(seeds, rad_list, hidden_units))

para_comb = list(itertools.product(seeds, hidden_units))
(seed, n_hidden) = para_comb[array_index-1]
#lr = round(lr, 6)
(lr, max_gn) = (0.015811, 10)
...
#             ]
    ...
    rad = (1/np.sqrt(3))
    num = (rad)**2
    cells = [EiRNNCell(input_features, n_hidden,max(n_hidden//(9+1),1),max(n_hidden//(9+1),1),nonlinearity=F.relu, i2h_init_policy=EiRNNCell_U_InitPolicy(), h2h_init_policy=EiRNNCell_W_InitPolicy(random=False)),
...
#             DenseLayer(n_hidden, n_output_classes, nonlinearity=None)] 
    rad = (1/np.sqrt(3))
    num = (rad)**2
    cells = [RNNCell(input_features, n_hidden, nonlinearity=F.relu, h2h_init_policy=W_UniformInit(num=num)),
            DenseLayer(n_hidden, n_output_classes, nonlinearity=None)]
...
rad = round(rad, 4)
filestr = str(params.get_exp_savepath() / f'{model_type}_icml_learning_curves_seed{seed}_hidden{n_hidden}_lr{lr}_GC{max_gn}_rad{rad}')
np.savez(filestr, **results)
torch.save(model.state_dict(), f'{PLAYGROUND_DIR}/saved_models/seq_mnist_shallow_test_{model_type}_model_seed{seed}_hidden{n_hidden}_lr{lr}_GC{max_gn}_rad{rad}_icml.pth')
