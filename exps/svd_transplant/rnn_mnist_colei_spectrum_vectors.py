...
from lib.update_policies import EiRNN_cSGD_UpdatePolicy, DalesANN_cSGD_UpdatePolicy, ColumnEiSGD_Clip, ColumnEiDenseSGD, SGD_Clip_UpdatePolicy, SGD_Clip_UpdatePolicy_NoClamp
from lib.init_policies import EiDenseWeightInit_WexMean, W_UniformInit, U_TorchInit, W_TorchInit, ColumnEiCell_W_InitPolicy, ColumnEi_FirstCell_U_InitPolicy, EiRNNCell_U_InitPolicy, EiRNNCell_W_InitPolicy, ColumnEi_Dense_InitPolicy, W_RNNInit_ColEI_Spectrum_Init, U_ColEIInit, W_ColEIInit_RNN_Spectrum_Init, Dense_RNNInit_ColEI_Spectrum_Init, Dense_RNNInit_ColEI_Spectrum_Vector, W_RNNInit_ColEI_Spectrum_Vectors, U_RNNInit_ColEI_Spectrum_Init,Dense_ColEIInit_RNN_Spectrum_Init,W_ColEIInit_RNN_Spectrum_Init,U_ColEIInit_RNN_Spectrum_Init,RNN_ColumnEi_FirstCell_U_InitPolicy
from lib import utils
from lib import rnn_basic_tasks
from lib.utils import acc_func
from config import PLAYGROUND_DIR

# results_dir = PLAYGROUND_DIR/"seq_mnist_sign_rnn_colei_spectrum_icml_rebuttal"
results_dir = PLAYGROUND_DIR/"seq_mnist_sign_ei_rnn_colei_spectrum_vectors"

def train(lr, n_epochs, cells, seed=0, max_gn=None, results_dir=results_dir, rad=None, ei=9):
...
#                                                                                         params.val_batch_size)
    ...
    filestr = str(params.get_exp_savepath() / f'sign_rnn_colei_spec_vec_learning_curves_ei{ei}_seed{seed}_lr{lr}_GC{max_gn}_rad{rad}')
...
seeds = [i for i in range(5)]
lr_list = [0.005, 0.01, 0.015]
gc_list = [1, 5, 10]
ei_ratio_list = [1, 9, 99]
para_comb = list(itertools.product(seeds, lr_list, gc_list, ei_ratio_list))
(seed, lr, max_gn, ei_ratio) = para_comb[array_index-1]
...
cells =[RNNCell(input_features, n_hidden, nonlinearity=F.relu, i2h_init_policy=RNN_ColumnEi_FirstCell_U_InitPolicy(dataset='MNIST'), h2h_init_policy=W_RNNInit_ColEI_Spectrum_Vectors(ratio=ei_ratio), update_policy=SGD_Clip_UpdatePolicy(max_grad_norm=max_gn)),
        DenseLayer(n_hidden, n_output_classes, nonlinearity=None, weight_init_policy=Dense_RNNInit_ColEI_Spectrum_Vector(ratio=ei_ratio), update_policy=SGD_Clip_UpdatePolicy(max_grad_norm=max_gn))] 

results, model = train(lr, n_epochs, cells, seed, max_gn=max_gn, rad=rad, ei=ei_ratio)
torch.save(model.state_dict(), f'{PLAYGROUND_DIR}/saved_models/seq_mnist_sign_rnn_colei_spec_vec_seed{seed}_lr{lr}_GC{max_gn}_rad{rad}.pth')
