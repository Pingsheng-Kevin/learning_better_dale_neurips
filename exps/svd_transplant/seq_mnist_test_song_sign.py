...
results_dir = PLAYGROUND_DIR/f"seq_mnist_test_{model_type}_sign_more"
...
if model_type == "song":
    # (lr, max_gn) = (0.010772, 10)
    (lr, max_gn) = (0.01, 10)
    cells = [ColumnEiCell(input_features, (n_hidden, 9),nonlinearity=F.relu, i2h_init_policy=ColumnEi_FirstCell_U_InitPolicy(dataset='MNIST'), h2h_init_policy=ColumnEiCell_W_InitPolicy(radius=1.5), update_policy=ColumnEiSGD_Clip(max=max_gn), clamp=False),
            ColumnEiDense(n_hidden, (n_output_classes, 9), nonlinearity=None, update_policy=ColumnEiDenseSGD(max=max_gn), clamp=False)
            ]
...
filestr = str(params.get_exp_savepath() / f'sign_{model_type}_icml_learning_curves_seed{seed}_lr{lr}_GC{max_gn}')
np.savez(filestr, **results)
torch.save(model.state_dict(), f'{PLAYGROUND_DIR}/saved_models/sign_seq_mnist_test_{model_type}_model_seed{seed}_hidden_lr{lr}_GC{max_gn}_icml.pth')
