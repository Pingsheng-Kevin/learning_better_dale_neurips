...
from lib.init_policies import EiDenseWeightInit_WexMean, W_UniformInit, U_TorchInit, W_TorchInit, ColumnEiCell_W_InitPolicy, ColumnEi_FirstCell_U_InitPolicy, EiRNNCell_U_InitPolicy, EiRNNCell_W_InitPolicy, ColumnEi_Dense_InitPolicy, W_RNNInit_ColEI_Spectrum, U_ColEIInit, W_ColEIInit_RNN_Spectrum, Dense_ColEISpectrum_Init
...
results_dir = PLAYGROUND_DIR/"seq_mnist_torch_rnn_colei_spectrum_icml_rebuttal"
...
#                                                                                         params.val_batch_size)
    ...
    filestr = str(params.get_exp_savepath() / f'torch_rnn_colei_spec_learning_curves_seed{seed}_lr{lr}_GC{max_gn}_rad{rad}')
...
cells =[RNNCell(input_features, n_hidden, nonlinearity=F.relu, i2h_init_policy=U_TorchInit(), h2h_init_policy=W_RNNInit_ColEI_Spectrum(), update_policy=SGD_Clip_UpdatePolicy(max_grad_norm=max_gn)), DenseLayer(n_hidden, n_output_classes, nonlinearity=None, update_policy=SGD_Clip_UpdatePolicy(max_grad_norm=max_gn), weight_init_policy=Dense_ColEISpectrum_Init())]     

results, model = train(lr, n_epochs, cells, seed, max_gn=max_gn, rad=rad)
torch.save(model.state_dict(), f'{PLAYGROUND_DIR}/saved_models/seq_mnist_rebuttal_torch_rnn_colei_spec_seed{seed}_lr{lr}_GC{max_gn}_rad{rad}.pth')
