...
# results_dir = PLAYGROUND_DIR/"seq_mnist_sign_colei_rnn_spectrum_icml_rebuttal"
results_dir = PLAYGROUND_DIR/"seq_mnist_sign_colei_rnn_spectrum_more"
...
#                                                                                         params.val_batch_size)
    ...
    filestr = str(params.get_exp_savepath() / f'sign_colei_rnn_spec_learning_curves_seed{seed}_lr{lr}_GC{max_gn}_rad{rad}')
...
cells =[RNNCell(input_features, n_hidden, nonlinearity=F.relu, i2h_init_policy=U_ColEIInit_RNN_Spectrum_Init(), h2h_init_policy=W_ColEIInit_RNN_Spectrum_Init(), update_policy=SGD_Clip_UpdatePolicy(max_grad_norm=max_gn)),
        DenseLayer(n_hidden, n_output_classes, nonlinearity=None, update_policy=SGD_Clip_UpdatePolicy(max_grad_norm=max_gn), weight_init_policy=Dense_ColEIInit_RNN_Spectrum_Init())]     

results, model = train(lr, n_epochs, cells, seed, max_gn=max_gn, rad=rad)
torch.save(model.state_dict(), f'{PLAYGROUND_DIR}/saved_models/seq_mnist_rebuttal_sign_colei_rnn_spec_seed{seed}_lr{lr}_GC{max_gn}_rad{rad}.pth')
