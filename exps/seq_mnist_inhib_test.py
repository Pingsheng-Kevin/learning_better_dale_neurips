...
results_dir = PLAYGROUND_DIR/f"seq_mnist_{model_type}_inhib_icml_test"
...
para_comb = list(itertools.product(seeds, inhib_ratios)) # 

(seed, inhib_ratio) = para_comb[array_index-1]


if model_type == "song":
    if inhib_ratio == 1: (lr, max_gn) = (0.010772, 10)
    if inhib_ratio == 9: (lr, max_gn) = (0.010772, 10)
    if inhib_ratio == 99: (lr, max_gn) = (0.010772, 5)
    cells = [ColumnEiCell(input_features, (n_hidden, inhib_ratio),nonlinearity=F.relu, i2h_init_policy=ColumnEi_FirstCell_U_InitPolicy(dataset='MNIST'), h2h_init_policy=ColumnEiCell_W_InitPolicy(radius=1.5), update_policy=ColumnEiSGD_Clip(max=max_gn)),
             ColumnEiDense(n_hidden, (n_output_classes, inhib_ratio), nonlinearity=None, update_policy=ColumnEiDenseSGD(max=max_gn))
            ]
elif "danns" == model_type:
    if inhib_ratio == 1: (lr, max_gn) = (0.010772, 10)
    if inhib_ratio == 9: (lr, max_gn) = (0.010772, 5)
    if inhib_ratio == 99: (lr, max_gn) = (0.023208, 5)
    cells = [EiRNNCell(input_features, n_hidden,max(n_hidden//(inhib_ratio+1),1),max(n_hidden//2,1),nonlinearity=F.relu, i2h_init_policy=EiRNNCell_U_InitPolicy(), h2h_init_policy=EiRNNCell_W_InitPolicy(numerator=1/3, random=False)),
            EiDense(n_hidden, n_output_classes, max(n_output_classes//(inhib_ratio+1),1), nonlinearity=None, weight_init_policy=EiDenseWeightInit_WexMean(numerator=2))
            ] 
    for cell in cells[-1:]:
...
filestr = str(params.get_exp_savepath() / f'{model_type}_icml_learning_curves_seed{seed}_inhib{inhib_ratio}_lr{lr}_GC{max_gn}')
np.savez(filestr, **results)
torch.save(model.state_dict(), f'{PLAYGROUND_DIR}/saved_models/{model_type}_model_test_seed{seed}_inhib{inhib_ratio}_lr{lr}_GC{max_gn}_icml.pth')