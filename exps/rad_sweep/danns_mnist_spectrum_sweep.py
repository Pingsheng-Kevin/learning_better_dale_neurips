...
from lib.update_policies import EiRNN_cSGD_UpdatePolicy, DalesANN_cSGD_UpdatePolicy
from lib.init_policies import EiRNNCell_W_InitPolicy
...
results_dir = PLAYGROUND_DIR/"seq_mnist_danns_neural_comp"
...
def train(lr, n_epochs, cells, seed=0, max_gn=None, results_dir=results_dir, rad=None):
    ...
    filestr = str(params.get_exp_savepath() / f'danns_rnn_learning_curves_seed{seed}_num_rec_cells{num_rec_cells}_hidden{n_hidden}_lr{lr}_GC{max_gn}_rad{rad}')
...
rad_list = [2/np.sqrt(3), 2*np.sqrt(2), 9, 10, 11, 12, 13]
...
if "ei" == model_type:
    ...
    for i in range(num_rec_cells-1): cells.append(EiRNNCell(n_hidden, n_hidden,n_hidden//10,n_hidden//10,nonlinearity=F.relu, h2h_init_policy=EiRNNCell_W_InitPolicy(numerator=num)))
    ...
    for cell in cells[:-1]:
        # cell.update_policy = EiRNN_cSGD_UpdatePolicy()
...
torch.save(model.state_dict(), f'{PLAYGROUND_DIR}/saved_models/danns_model_seed{seed}_num_rec_cells{num_rec_cells}_hidden{n_hidden}_lr{lr}_GC{max_gn}_rad{rad}.pth')
